    ...
    <div className="space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between mb-4 px-1">
          <h3 className="text-[15px] font-semibold text-white/80 tracking-tight">History</h3>
          <span className="text-white/40 text-[12px]">{filteredTransactions.length} results</span>
        </div>
      ...
      <IosSheet isOpen={isFilterOpen} onClose={() => setIsFilterOpen(false)} title="Filter Transactions">
        <div className="space-y-6">
          {/* Type Filter */}
          <div>
            <h4 className="text-white/60 text-sm font-medium mb-3">Transaction Type</h4>
          ...
          {/* Date Filter */}
          <div>
            <h4 className="text-white/60 text-sm font-medium mb-3">Time Period</h4>
            <div className="flex bg-[#1C1C1E]/70 p-1 rounded-2xl border border-white/10">
              {(['all', 'day', 'week', 'month', 'year'] as const).map(date => (
                <button
                  key={date}
