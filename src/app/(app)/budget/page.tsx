...
const getCategoryIcon = (iconName: string) => {
  switch (iconName) {
    case "hamburger":
      return <ForkKnife size={20} weight="duotone" />;
    case "car":
      return <CarProfile size={20} weight="duotone" />;
    case "money":
      return <Money size={20} weight="duotone" />;
    case "shopping":
      return <ShoppingBag size={20} weight="duotone" />;
    default:
      return <Tag size={20} weight="duotone" />;
  }
};
...
export default function BudgetPage() {
  ...
  const percentage =
    Math.min(totalBudget > 0 ? (totalSpent / totalBudget) * 100 : 0, 100) || 0;
  ...
  return (
    <div className="space-y-6">
      {/* Total Budget Card */}
      <section className="bg-gradient-to-br from-[#1C1C1E]/70 to-[#2C2C2E]/70 backdrop-blur-2xl p-5 rounded-[20px] border border-white/10 shadow-2xl relative overflow-hidden">
        <div className="flex justify-between items-start mb-6 relative z-10">
          ...
          <button
            ...
            className="bg-white/10 text-white p-2.5 rounded-full hover:bg-white/20 transition-colors backdrop-blur-md"
          >
      ...
      <section>
        ...
        <div className="bg-gradient-to-br from-[#1C1C1E]/70 to-[#2C2C2E]/70 backdrop-blur-2xl p-2 rounded-[20px] border border-white/10">
          {budgets.length === 0 ? (
            <div className="py-8 px-4 text-center text-white/40 text-sm">
              No budgets created yet. Tap the{" "}
              <span className="text-white/60 font-medium">+</span> button to
              create one.
            </div>
          ) : (
            budgets.map((budget) => (
