...
export const TransactionList = ({ limit, transactions: customTransactions }: TransactionListProps = {}) => {
  ...
  const displayTransactions = limit ? sourceTransactions.slice(0, limit) : sourceTransactions;

  const groupedTransactions = useMemo(() => {
    return displayTransactions.reduce((acc, trx) => {
      const dateObj = new Date(trx.date);
      const dateHeader = dateObj.toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
      if (!acc[dateHeader]) {
        acc[dateHeader] = [];
      }
        ...
        Object.entries(groupedTransactions).map(([dateHeader, group]) => (
          <React.Fragment key={dateHeader}>
            <div className="bg-white/5 px-5 py-2 border-b border-white/5 backdrop-blur-md sticky top-0 z-10 flex items-center">
              <span className="text-[12px] font-semibold text-white/60 tracking-wider uppercase">{dateHeader}</span>
            </div>
            {group.map((trx) => {
