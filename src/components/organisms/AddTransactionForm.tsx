    ...
    <form onSubmit={handleSubmit} className="space-y-6 w-full">
      ...
      <div className="bg-gradient-to-br from-[#1C1C1E]/70 to-[#2C2C2E]/70 backdrop-blur-2xl rounded-[20px] overflow-hidden border border-white/10 shadow-2xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/5 focus-within:bg-white/5 transition-colors">
          <span className="text-[17px] text-white tracking-tight shrink-0">
            Amount ({currency})
          </span>
          <input
            type="text"
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/5 focus-within:bg-white/5 transition-colors relative">
          ...
          <select
            ...
            className="bg-transparent text-right text-[17px] text-white/70 outline-none w-full ml-4 appearance-none cursor-pointer pr-4 relative z-10"
            dir="rtl"
          >
            <option
              value="expense"
              className="bg-[#1C1C1E] text-white text-left"
            >
              Expense
            </option>
            <option
          ...
          <CaretRight
            size={16}
            weight="bold"
            className="text-white/30 absolute right-5 top-1/2 -translate-y-1/2 pointer-events-none"
          />
        </div>
        ...
        <div className="flex items-center justify-between p-5 border-b border-white/5 focus-within:bg-white/5 transition-colors relative">
          ...
          >
            <option
              value=""
              disabled
              className="bg-[#1C1C1E] text-white/30 text-left"
            >
              Select Wallet
            </option>
            {wallets.map((w) => (
      ...
      <Button
        type="submit"
        className="w-full mt-4 bg-white text-black hover:bg-gray-200 py-6 text-[17px] rounded-[16px] shadow-xl"
        disabled={
          isLoading ||
          !walletId ||
          !categoryId ||
